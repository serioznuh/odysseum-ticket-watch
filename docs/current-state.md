...
- **Send permission is shared** — before calling Telegram a pass must win a reservation on `refs/heads/runtime-state` for every logical delivery its message carries: each member key with its episode identity, so a finding the two hosts group differently still has one owner. The reservation is a commit on the fetched tip pushed as a plain fast-forward, which the remote accepts only while that tip is current; a racing host is rejected, re-reads the winner's entry and declines. A declined, lost, unanswered (bounded Git timeout) or refused (missing ref) reservation sends nothing and leaves the work pending with its baselines unmoved, so a Mac that wakes with a failed pre-run sync still reads the live ref and cannot resend what the cloud published, while unrelated news and reminders go ahead.
  Only the holding process moves its reservation: `released` after a definite failure or a 10-min lease spent before the send began (a laptop that slept mid-pass never sends late), `uncertain` once Telegram may have been called, saved atomically with `sending`. No host takes over another's reservation, whatever its lease or clock says: an unreleased one quarantines that work like any uncertain attempt until its receipt, release or expiry. Only the same store (holder id in `.cache/state-sync/holder`) may take over its own, after the lease, when its live file proves that earlier pass never reached Telegram, at the next generation through the same push. Receipts settle and prune reservations; recovery unions them. Dry runs and state files outside the store stay uncoordinated.
  A reservation's Git child that outlives cleanup is recorded like a sync's and exits the watcher 5/6; `local-check.sh` then stops before its post-run sync. Schema 5 adds `reservations`; older state migrates with every receipt, and code still on schema 4 cannot sync the upgraded ref and reports it as a sync failure.
...
- **Nothing in a local firing waits forever** — the deployment pull runs under the same stdlib boundary the syncs use (no platform `timeout` binary); a pull that never answers is a failed deployment, so the firing continues on the installed code. `locked` supervises the whole firing under an overall deadline of two launchd intervals plus a cleanup allowance, well clear of a healthy pass. On that deadline, or when launchd or Ctrl-C stops the supervisor, it stops and reaps only this run's own process tree — each child leads its own session, so a wedged `git-remote-https` or `ssh` goes too — and only then releases the lock, exiting 4. Each supervisor in the chain hands a stop it receives down to the tree it owns — a state sync forwards it to the Git child it started in a session of its own, inside the allowance the level above gives it — so no Git process outlives the firing that started it. The lock file is never deleted to break a lock, so the next firing cannot race a live writer, and a hung tree costs the ladder two firings instead of every later one. A tree that outlives repeated SIGKILL rounds is never *presumed* gone — an advisory lock dies with its holder, so the firing records that group next to the lock and exits 5, the next firing stops on that record rather than starting a second writer, and the record clears itself once the group is really gone (with one durable marker, so the owner hears about a machine in that state). Exit 5 is a hard stop in **both** wrappers wherever it appears — the local one at deployment and either sync, the cloud one at its pre-run sync, which then skips the watcher, the credential probe and the post-run sync — exactly like the missing-ref exit, never folded into "failed, carry on", and it cannot be masked by an earlier ordinary failure; if the marker itself cannot be written, the record goes into the lock file the firing already holds — rewriting a file that exists needs no new inode or directory change, and that file is truncated, never deleted — and while neither lands the firing keeps holding the lock rather than releasing it unguarded — for as long as that level actually has: a nested sync or pull only gets the allowance reserved for recording, because holding past it just means being killed before its status can reach the level that acts on it, while the lock holder may use whatever is left of the firing's documented lifetime and no more. Running out of that allowance with the group still alive and still unrecorded reports a **different** status (6, blocking in both wrappers too) rather than claiming a guard it does not have. A nested level in that state also announces the group on a pipe `locked` hands down, because its exit status cannot reach the lock holder once the watchdog, launchd or Ctrl-C has stopped the shell in between; the lock holder, the last level that can act, records it before releasing the lock — by name when it knows exactly one such group (so the record clears itself), otherwise unnamed, which blocks every later firing until a human clears it.
